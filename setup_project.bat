...
:: Remove any incorrectly created folders first
if exist mustdo (
    echo Removing incorrectly placed mustdo folder...
    rmdir /s /q mustdo
)
if exist randos (
    echo Removing incorrectly placed randos folder...
    rmdir /s /q randos
)
if exist fillers (
    echo Removing incorrectly placed fillers folder...
    rmdir /s /q fillers
)

:: Create main directories in correct structure
for %%d in (
    "core"
    "gui" 
    "tools"
