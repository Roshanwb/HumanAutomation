...
def click_component(component_id):
    ...
    log_action(f"Clicked on component '{component_id}' at ({x}, {y})")
     ...
     # Only update last Chrome tab if not in special login
...
def check_special_login():
    ...
    if ("03:01" < current_time.strftime("%H:%M") < "03:15") and not special_login_executed:
        ...
        if saved_scenario:
            ...
            click_last_chrome_tab()  # Ensure the last clicked Chrome tab is focused
...
def execute_scenario_from_line(scenario_file, start_line):
    ...
    for index, line in enumerate(lines[start_line:], start=start_line):
        ...
        if command.startswith("click"):
            ...
            continue  # Ignore TOTALTIME when resuming
